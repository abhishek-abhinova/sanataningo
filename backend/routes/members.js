...
router.post('/register', upload.fields([
  ...
  try {
    ...
    const memberData = {
      ...
      aadhaar_front_image: req.files?.aadhaarFront ? req.files.aadhaarFront[0].path : '',
      aadhaar_back_image: req.files?.aadhaarBack ? req.files.aadhaarBack[0].path : '',
      membership_type: resolvedMembershipPlan,
      membership_fee: resolvedAmount,
      payment_status: razorpay_payment_id ? 'completed' : 'pending',
      payment_id: req.files?.paymentScreenshot ? req.files.paymentScreenshot[0].path : '',
