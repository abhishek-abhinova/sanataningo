...
const sendDonationReceiptWithPDF = async (donation, receiptPath) => {
  try {
    const donationData = normalizeDonationData(donation);
    console.log('📧 Preparing email with PDF receipt for:', donationData.email);
    ...
    // Ensure "from" address matches SMTP_USER exactly (required by SMTP providers)
    const fromEmail = process.env.SMTP_USER; // Must match SMTP_USER exactly
    const fromName = process.env.ORG_NAME || 'Sarbo Shakti Sonatani Sangathan';
    
    // Validate that SMTP_USER is set
    if (!fromEmail) {
      throw new Error('SMTP_USER environment variable is not set. Please set it in your .env file.');
    }
    ...
    const receiptFilename = `donation-receipt-${donationData.receiptNumber}.pdf`;
    
    const mailOptions = {
      from: `"${fromName}" <${fromEmail}>`,
      replyTo: process.env.ORG_EMAIL || fromEmail,
      to: donationData.email,
      subject: `🙏 Donation Receipt – ${donationData.receiptNumber}`,
      html: `
        <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto; padding: 20px; background: linear-gradient(135deg, #fff8f0, #fef6ed); border-radius: 10px;">
          ...
          <div style="background: white; padding: 25px; border-radius: 10px; box-shadow: 0 4px 15px rgba(0,0,0,0.1);">
            ...
            <p style="color: #555; line-height: 1.6;">📎 Your official donation receipt (PDF) is attached to this email for your records. This receipt is eligible for tax exemption under Section 80G of the Income Tax Act.</p>
    ...
    // Use sendMailWithRetry for better reliability
    ...
    console.log('✅ Donation receipt email sent successfully to:', donationData.email);
